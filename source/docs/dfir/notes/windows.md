...
Command references for Windows forensic analysis, covering system queries, user activity, persistence mechanisms, network connections, and log collection.
...
Threat actors can manipulate scheduled tasks such that Task Scheduler no longer has visibility of the recurring task.
...
Then kill these Registry schtask entries via Regedit's GUI.
...
```powershell
get-content evil.ps1 -stream "evil_stream"
```
...
HKCU signifies Current User and results will be limited to the user you are. To see more results, change HKCU to HKU.
