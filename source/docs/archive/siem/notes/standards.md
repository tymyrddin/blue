...
diagrams consisting of one root node, leaves, and children nodes. Bottom to top, child nodes are conditions whose satisfaction makes the direct parent node true. An attack is considered complete when the root is satisfied. 
Each node is satisfied only by its direct child nodes.

| ![Attack tree Attack on a password](/_static/images/attack-tree.png) |
|:-------------------------------------------------------------------------:|
|                Attack on a password. Credit: Rajesh Kumar                 |

Suppose there is 1 grandchild below the root node. In such a case multiple steps are needed to carry out an attack: the grandchild’s conditions need to be satisfied for the direct parent node to be true, and then the direct parent node condition for the root node to be true. It also has `AND` and `OR` options which represent alternatives and different steps towards achieving that goal.
