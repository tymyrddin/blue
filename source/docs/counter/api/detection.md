...
Capture a structured log entry for every API request containing at minimum: the timestamp,
...
(where introspection is correctly disabled, that is) or an active recon attempt.
...
export (which is tracked and authorised in a well-run environment) or data exfiltration via the API.
