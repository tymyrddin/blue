...
or process has any reason to touch a honeypot.
...
Files that look valuable make the share more convincing: spreadsheets with financial data
...
Effective placement follows the threat model. If the concern is an attacker who has
already achieved initial access and is moving laterally, deception assets along the
likely lateral movement path are most effective.
