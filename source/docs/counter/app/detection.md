...
Capture a structured log entry for every request containing: the timestamp, the resolved
...
Take the source IP from the network layer, not from `X-Forwarded-For`. Recording an
...
more useful for detection than successful ones, but both are worth capturing.
