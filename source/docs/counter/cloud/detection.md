...
GCP produces Admin Activity logs (always on, cannot be disabled) and Data Access logs (require
explicit enablement per service).
...
- `storage.objects.get` calls from external IP addresses (for buckets that are only expected to be accessed
  internally)
