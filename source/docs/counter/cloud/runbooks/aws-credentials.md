...
Check CloudTrail for unauthorised activity:
...
```bash
# Delete unauthorised IAM users
aws iam delete-user --user-name attacker-created-user

# Delete unauthorised access keys
...
- Unauthorised resources terminated
- Access revoked
...
Business impact:
- Unauthorised AWS costs: $\[amount] (estimated)
...
- Terminated unauthorised resources
...
Costs:
- Unauthorised resource usage: $\[amount]
...
- Potentially waiving costs from unauthorised usage (case by case)
...
If yes: Preserve CloudTrail logs, EC2 snapshots, and all evidence before clean-up.

### Do we report this?

Contact AWS Support if:
- Root account compromised (always report this)
- Need help identifying unauthorised resources
- Want to request cost forgiveness for unauthorised usage
