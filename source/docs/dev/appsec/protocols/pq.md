...
Separate from post-quantum concerns, TLS 1.3 has been subject to timing side-channel research (Raccoon, Lucky13 variants in specific implementations). These affect specific implementations rather than the protocol itself; patching the TLS library to a current version is the appropriate response.

## Practical readiness

For most applications, the immediate action is to inventory where RSA or EC keys protect data with long-term sensitivity, and to check whether the TLS library and key exchange configuration in use support hybrid ML-KEM/X25519 when it becomes available in stable releases. NIST and CISA have published migration guidance; the timeline for mandatory migration in regulated environments is being established on a sector-by-sector basis.
