...
Where data physically lives has legal, contractual, and reputational dimensions for a Dutch
...
The default tenant region for a Dutch organisation onboarding to M365 should be the EU, but
...
on what is and is not covered. For most purposes, a Dutch non-profit operating under a
standard M365 agreement with an EU tenant and a current DPA with Microsoft is in a
defensible position.

## The Azure question

If the organisation uses Azure services (beyond what is embedded in M365), the data residency
of each resource is determined by the region it is deployed in. Azure West Europe (Netherlands)
and North Europe (Ireland) are both EU. If any Azure resources are deployed in US regions,
that needs to be reviewed against the data they process.
...
Dutch-headquartered vendors such as AFAS (data centres in Almere) offer a clear and
straightforward answer. US-based SaaS vendors with EU data centre options offer a more
complex answer that depends on configuration and the specific service. Consumer AI tools
without enterprise DPAs offer no acceptable answer for personal data.
