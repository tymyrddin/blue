...
includes security as one of its requirements. The security architect's role in GDPR compliance
is to ensure that the technical and organisational measures are appropriate for the data being
...
data about members' household members that was never necessary, creates GDPR exposure.
...
the GDPR to appoint a Data Protection Officer (DPO). If one is in place, your work as a
security architect should be coordinated with them. If one is not in place and should be,
...
The GDPR requires a register of processing activities. If one exists, it should inform your
application landscape work. If it does not exist, creating it is a project that involves
legal, HR, fundraising, and IT. The security architect's input is on the technical side:
where does the data live, how is it protected, who has access.
