...
* A team benefits from thinking through how to manage communications, software, hardware, and any other available resources such as port lists and network diagrams.
* Familiarity with frameworks, standards, and the main recommended practices for securing networks and systems is worth building early.
* Team members who know the common attack vectors can identify attacks through certain types of activity.
...
Hardware and software to analyse an incident is worth installing in advance. This can include a [SIEM stack](siem.md), forensic workstations, backup drives, disk images to preserve log files and incident data, laptops for live analysis and report writing, and packet sniffers to track network activity.
...
For evidence to be admissible in court, documenting how it was acquired and preserved is essential. All evidence activity is recorded in a detailed log that includes identifying information: location, hostname, MAC, and IP addresses. The log also covers the names, titles, and contact numbers of each individual who collected or handled the evidence. Timestamps are key to any investigation and belong in the log too, particularly when handling evidence. The storage location goes in separately from the identifying information, which refers to where the evidence was collected rather than stored.
