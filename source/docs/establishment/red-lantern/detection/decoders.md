...
The Establishment keeps its custom decoders in `/var/ossec/etc/decoders/`. These files persist across 
Wazuh updates, unlike the default rule files which might be overwritten.

After making changes, the Wazuh manager requires a restart for them to take effect
