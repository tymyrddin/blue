...
[TLS](encryption/tls.md) encrypts traffic between the browser and the server. Versions older than 1.2 carry
well-documented weaknesses and are worth disabling; TLS 1.3 is preferred where the deployment allows it, partly because
it does not require manually curating [cipher suites](encryption/cipher.md).
...
[HSTS](http/hsts.md) closes the HTTP fallback gap. Once a browser has seen the `Strict-Transport-Security` header over
...
[CSP](http/csp.md) defines which sources the browser is permitted to load and execute for a given page: scripts, styles,
...
[`X-Content-Type-Options: nosniff`](http/xcontent.md) prevents browsers from guessing a resource's MIME type when the
server has declared one. Without it, a browser may execute a file served as `text/plain` as JavaScript if the content
looks like script.

This matters for CSP specifically. A CSP that allows `'self'` for scripts trusts content from the same origin. If the
...
[X-Frame-Options](http/xframe.md) controls whether the page can be embedded in an iframe, protecting against
...
Three attributes on the [`Set-Cookie`](http/cookie.md) header cover distinct attack vectors.
...
[CORS](cors/requirements.md) is the browser's enforcement mechanism for cross-origin requests. A strict CORS policy
