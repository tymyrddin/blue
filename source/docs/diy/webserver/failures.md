...
Disabling RSA key exchange and requiring ECDHE or DHE ciphers means each session uses an ephemeral key that is never stored. TLS 1.3 provides this automatically; TLS 1.2 requires [explicit cipher suite configuration](encryption/forward-secrecy.md).
