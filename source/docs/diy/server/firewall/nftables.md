...
Named sets separate port and user definitions from the chain logic, making them straightforward to extend. The policy is default-drop on input and output, with explicit allows for the services listed in the sets.
...
table inet filter {
    ...
    set tcp_port_out { type inet_service; 
        elements = { 
            53,    # DNS
    ...
    set tcp_port_in { type inet_service; 
        elements = { 
            2200,  # SSH
        }
    }
    
    set udp_port_out { type inet_service;  
        elements = { 
            53,    # DNS
            123,   # NTP
        }
    ...
    set udp_port_in { type inet_service; 
        elements = {
        }
    }
    
    set user_out { type uid;      
        elements = {
        }
    }
    
    set user_in { type uid;        
        elements = {
        }
    }
...
Most distributions ship nftables support compiled in. This section applies only when building a custom kernel.
