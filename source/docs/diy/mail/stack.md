...
[SPF](runbooks/spf.md), [DKIM](runbooks/dkim.md), and [DMARC](runbooks/dmarc.md) work together here. Each closes a gap the
others leave open.
...
[STARTTLS](runbooks/mail-tls.md) upgrades a plaintext SMTP connection to an encrypted one. The upgrade is opportunistic:
...
Both DANE and MTA-STS close the STARTTLS downgrade gap, but by different means.
